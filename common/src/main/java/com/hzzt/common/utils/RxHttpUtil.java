...
import com.hzzt.common.data.http.ExceptionHandle;
...
//                        .map(new HandleFuc<T>())  //这里可以取出BaseResponse中的Result
    ...
    private static class HandleFuc<T> implements Function<CommonResponse<T>, T> {
        @Override
        public T apply(CommonResponse<T> response) {
            if (!response.isOk())
                throw new RuntimeException(!"".equals(response.getErrorCode() + "" + response.getMsg()) ? response.getMsg() : "");
